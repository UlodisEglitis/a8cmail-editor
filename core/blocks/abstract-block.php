...
abstract class AbstractBlock {
	...
	protected function getAsset(string $asset_name, bool $as_url = false) {
		$base = EMAIL_EDITOR_DEMO_PATH;
		if ( $as_url ) {
			$base = EMAIL_EDITOR_DEMO_URL;
		}
		return 	$base . "/core/blocks/{$this->blockName}/build/$asset_name";
	}
	...
	protected function registerBlockType() {
		...
		$metadata_path = $this->getAsset('block.json');
	...
	protected function getEditorScript($key = null) {
		$asset_file_path = $this->getAsset('index.asset.php');
		...
		$script = [
			'handle' => 'a8cmail-' . $this->blockName . '-block',
			'path' => $this->getAsset('index.js', true),
	...
	protected function getEditorStyle($key = null) {
		$style_name = "index.css";
		$path = $this->getAsset( $style_name );
		...
		$style = [
			'handle' => 'a8cmail-' . $this->blockName . '-block',
			'path' => $this->getAsset( $style_name, true )
		];
